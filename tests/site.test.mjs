...
test('branding and locales',()=>{assert.equal(DOMAIN,'vikub.group');assert.equal(defaultLocale,'ru');assert.deepEqual(locales,['ru','it','en']);assert.equal(typeof BASE_PATH,'string');});
test('unconfigured integrations remain genuinely disabled',()=>{assert.deepEqual(validateIntegrations({}),{googleAnalyticsId:'',contactFormEndpoint:'',registrationUrl:''});});
...
test('missing timezone in a timestamp fails',()=>assert.throws(()=>validateSeminar({...seminar,startAt:'2026-10-01T11:00:00'})));
test('registration cannot close after the start',()=>assert.throws(()=>validateSeminar({...seminar,registrationDeadline:'2026-10-02T11:00:00+02:00'})));
...
test('11:00 Italian time on 1 October 2026 is preserved',()=>{const parts=new Intl.DateTimeFormat('en-GB',{timeZone:seminar.timeZone,hour:'2-digit',minute:'2-digit',hourCycle:'h23'}).format(new Date(seminar.startAt));assert.equal(parts,'11:00');assert.equal(new Date(seminar.startAt).toISOString(),'2026-10-01T09:00:00.000Z');});
...
for(const locale of locales){
 ...
 test(`${locale}: no invented contacts or identities`,()=>{const settings=json(`../src/content/settings/${locale}.json`);assert.equal(settings.company.name,'Vikub Group');assert.equal(settings.offices[0].person,'Mario Ubaldi');assert.ok(settings.offices[0].address.includes('via Carlo Gozzi, 8'));assert.ok(!JSON.stringify(settings).includes('000-00-00'));assert.deepEqual(settings.socials.map(s=>s.type),['linkedin','telegram']);});
 test(`${locale}: required home and event content`,()=>{const home=json(`../src/content/pages/${locale}/home.json`);const events=json(`../src/content/pages/${locale}/events.json`);assert.equal(home.whatWeDo.cards.length,4);assert.equal(home.hero.secondaryCta.page,'events');assert.ok(home.seminar.title.includes('{date}'));assert.ok(events.program.length>=1);assert.ok(events.registration.unavailable);assert.ok(events.lead.includes('{date}'));});
}
test('impossible calendar dates are rejected rather than normalized',()=>{
  assert.throws(()=>validateSeminar({...seminar,startAt:'2027-02-30T11:00:00+01:00'}),/календарную/);
});
